...
function TrendBadge({ trend }: { trend: Trend }) {
  ...
  if (trend.neutral) return <span className={`${base} bg-white/8 text-white/40`}>{trend.value}</span>
...
const COLOR_MAP: Record<StatColor, { icon: string; pill: string; glow: string }> = {
  ...
  blue:   { icon: 'bg-white/8 text-white/60',             pill: 'bg-white/8 text-white/60',             glow: 'shadow-white/5' },
  ...
  return (
    <Card className={`glass transition-all duration-200 rounded-2xl shadow-lg ${c.glow}`}>
      <CardContent className="pt-4 pb-4 px-4">
        ...
        <p className="text-[11px] text-white/30 mb-1 uppercase tracking-wide">{label}</p>
        <p className={`text-2xl font-bold text-white leading-none ${mono ? 'font-mono' : ''}`}>{value}</p>
        {subValue && <p className="text-xs text-white/20 mt-1.5 font-mono">{subValue}</p>}
      </CardContent>
    </Card>
  )
}
