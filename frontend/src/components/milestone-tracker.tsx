...
const STATUS_CONFIG = {
  ...
  submitted: { icon: Clock, color: 'text-amber-500', bg: 'bg-amber-500', label: 'Submitted' },
  active: { icon: Circle, color: 'text-amber-500', bg: 'bg-amber-500/20 border-2 border-amber-500', label: 'Active' },
  disputed: { icon: AlertTriangle, color: 'text-red-500', bg: 'bg-red-500', label: 'Disputed' },
  pending: { icon: Circle, color: 'text-slate-600', bg: 'bg-slate-700', label: 'Pending' },
}
          ...
          <div key={milestone.id} className="flex gap-4">
            <div className="flex flex-col items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${config.bg}`}>
                <Icon className={`w-4 h-4 ${milestone.status === 'active' ? 'text-amber-500' : 'text-white'}`} />
              </div>
              {!isLast && <div className="w-0.5 bg-slate-700 flex-1 my-1 min-h-[2rem]" />}
            </div>
            <div className={`pb-6 ${isLast ? '' : ''}`}>
              ...
              <p className="text-amber-500 font-mono text-sm mt-0.5">{milestone.amount.toLocaleString()} XLM</p>
            </div>
          </div>
        )
      })}
